...
export default function ClientesView() {
  ...
  const [sortBy, setSortBy] = useState<"nombre" | "citas" | "gasto" | "reciente">("reciente");
  ...
  const filtered = clientes
    ...
    .sort((a, b) => {
      ...
      if (sortBy === "citas") return b.total_citas - a.total_citas;
      if (sortBy === "gasto") return parseFloat(b.total_gastado) - parseFloat(a.total_gastado);
      return 0; // "reciente" — backend already returns newest first
    });
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      {/* ── Lista ── */}
      <div
        className={`flex flex-col bg-gray-50 dark:bg-gray-950 overflow-hidden transition-all ${selected ? "hidden md:flex md:w-80 md:shrink-0 border-r border-gray-100 dark:border-gray-800" : "flex-1"}`}
      >
        {/* Header */}
        <div className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 p-4 md:p-5">
          ...
          <div className="relative mb-2">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500"
            />
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">Ordenar:</span>
            {(["reciente", "nombre", "citas", "gasto"] as const).map((opt) => {
              const labels = { reciente: "Más reciente", nombre: "Nombre A-Z", citas: "Más citas", gasto: "Más gasto" };
              return (
                <button
                  key={opt}
                  onClick={() => setSortBy(opt)}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${sortBy === opt ? "bg-green-600 text-white border-green-600" : "border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800"}`}
                >
                  {labels[opt]}
                </button>
              );
          ...
          {loading || isLoadingNegocioId ? (
            ...
            filtered.map((c) => (
              <div
                ...
                className={`bg-white dark:bg-gray-900 rounded-2xl border p-4 cursor-pointer transition-all hover:border-green-300 hover:shadow-sm ${selected?.id === c.id ? "border-green-400 ring-1 ring-green-200 shadow-sm" : "border-gray-100 dark:border-gray-800"}`}
              >
                <div className="flex items-center gap-3">
      ...
      {(selected || loadingDetalle) && (
        <div className="flex-1 overflow-y-auto bg-white dark:bg-gray-900">
          {loadingDetalle ? (
            ...
            <>
              {/* Breadcrumb */}
              <div className="px-6 pt-5 pb-2 flex items-center gap-2 text-sm border-b border-gray-50 dark:border-gray-800">
                <button
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 px-3 py-2 rounded-xl text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-100 dark:border-gray-800">
                            {[
                              ...
                              <th
                                key={h}
                                className="text-left text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider pb-3 pr-4"
                              >
                                {h}
                              </th>
                            ))}
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                          {selected.historial.map((v) => (
                            ...
                            >
                              <td className="py-3 pr-4 text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                {formatFecha(v.hora_inicio)}
                              </td>
                              <td className="py-3 pr-4 text-xs font-medium text-gray-800 dark:text-gray-200">
                                {v.servicio_nombre}
                              </td>
                              <td className="py-3 pr-4 text-xs text-gray-600 dark:text-gray-400">
                                {v.empleado_nombre}
                              </td>
                              <td className="py-3 pr-4 text-xs font-semibold text-gray-800 dark:text-gray-200">
                                {v.monto_anticipo}
                              </td>
                              <td className="py-3">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800">
              ...
              <button
                ...
                className="h-8 w-8 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-green-500"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1.5">
            ...
            <div className="flex gap-3 p-6 pt-0">
              <button
                onClick={() => setShowModal(false)}
                className="flex-1 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-semibold py-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 text-sm"
              >
                Cancelar
              </button>
              <button
